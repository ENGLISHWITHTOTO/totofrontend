...
export function CourseBuilder() {
  ...
  ): Array<{ id: string; title: string; type: string }> | null => {
    for (const item of structure) {
      if (item.type === "lesson" && item.lesson?.id === lessonId) {
        return [
          ...currentPath,
          { id: item.id, title: item.title, type: "lesson" },
        ];
      }
      if (item.children) {
        const result = buildLessonPath(item.children, lessonId, [
          ...currentPath,
          { id: item.id, title: item.title, type: "subcategory" },
        ]);
          ...
          return items.map((item) => {
            if (
              ...
              return {
                ...
                lesson: {
                  ...item.lesson,
                  blocks: item.lesson.blocks.filter(
  ...
  const readingWritingBlocks = [
    ...
    {
      type: "write_sentence_given_words",
      icon: Edit,
      label: "Write Sentence Using Words",
  ...
  const grammarVocabBlocks = [
    ...
    {
      type: "choose_best_title",
      icon: FileText,
      label: "Choose Best Title",
  ...
  const renderStructureItem = (item: any, courseId: string, level: number) => {
    ...
    const countContent = (
      items: any[]
    ): { lessons: number; subcategories: number } => {
      let lessons = 0;
      let subcategories = 0;
      items.forEach((item) => {
    ...
    const childCounts = hasChildren
      ? countContent(item.children)
      : { lessons: 0, subcategories: 0 };
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.type === "lesson" && item.lesson?.blocks && (
                  <span className="text-xs text-muted-foreground">
                    {item.lesson.blocks.length}{" "}
                    {item.lesson.blocks.length === 1 ? "block" : "blocks"}
                  </span>
                )}
                {item.type === "subcategory" && !hasChildren && (
                  <span className="text-xs text-muted-foreground">
        ...
        {item.children && isExpanded && (
          <div className="space-y-2 mt-2">
            {item.children.map((child: any) =>
              renderStructureItem(child, courseId, level + 1)
            )}
  ...
  const renderBlockCategory = (
    ...
    const isExpanded =
      expandedCategories.includes(categoryKey) || searchQuery.length > 0;
        ...
        <div className="sticky top-0 bg-card/95 backdrop-blur pb-3 z-10">
          ...
          {searchQuery && (
            <p className="text-xs text-muted-foreground mt-2 px-1">
              {totalFilteredCount}{" "}
              {totalFilteredCount === 1 ? "block" : "blocks"} found
            </p>
          )}
        </div>
    ...
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 border border-border rounded-xl bg-gradient-to-r from-card to-card/50">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <Badge
            variant={
              selectedLesson?.status === "live" ? "default" : "secondary"
            }
            className={
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              toast.info("Version management coming soon")
            }
          >
          ...
          <div className="space-y-4 text-left">
            {selectedLesson.blocks.map((block, index) => {
              const blockTypeInfo = {
                // Content blocks
                video: {
                  icon: BLOCK_TYPE_INFO.video.icon,
                  label: BLOCK_TYPE_INFO.video.label,
                  color: "text-blue-600",
                },
                text: {
                  icon: BLOCK_TYPE_INFO.text.icon,
                  label: BLOCK_TYPE_INFO.text.label,
                  color: "text-green-600",
                },
                image: {
                  icon: BLOCK_TYPE_INFO.image.icon,
                  label: BLOCK_TYPE_INFO.image.label,
                  color: "text-purple-600",
                },
                audio: {
                  icon: BLOCK_TYPE_INFO.audio.icon,
                  label: BLOCK_TYPE_INFO.audio.label,
                  color: "text-orange-600",
                },
                attachment: {
                  icon: BLOCK_TYPE_INFO.attachment.icon,
                  label: BLOCK_TYPE_INFO.attachment.label,
                  color: "text-gray-600",
                },

                // Reading & Writing
                mcq_single: {
                  icon: BLOCK_TYPE_INFO.mcq_single.icon,
                  label: BLOCK_TYPE_INFO.mcq_single.label,
                  color: "text-green-600",
                },
                mcq_multiple: {
                  icon: BLOCK_TYPE_INFO.mcq_multiple.icon,
                  label: BLOCK_TYPE_INFO.mcq_multiple.label,
                  color: "text-green-700",
                },
                fill_blanks: {
                  icon: BLOCK_TYPE_INFO.fill_blanks.icon,
                  label: BLOCK_TYPE_INFO.fill_blanks.label,
                  color: "text-green-500",
                },
                sentence_completion: {
                  icon: BLOCK_TYPE_INFO.sentence_completion.icon,
                  label: BLOCK_TYPE_INFO.sentence_completion.label,
                  color: "text-green-400",
                },
                matching: {
                  icon: BLOCK_TYPE_INFO.matching.icon,
                  label: BLOCK_TYPE_INFO.matching.label,
                  color: "text-green-800",
                },
                summary_completion: {
                  icon: BLOCK_TYPE_INFO.summary_completion.icon,
                  label: BLOCK_TYPE_INFO.summary_completion.label,
                  color: "text-green-600",
                },
                table_completion: {
                  icon: BLOCK_TYPE_INFO.table_completion.icon,
                  label: BLOCK_TYPE_INFO.table_completion.label,
                  color: "text-green-700",
                },
                flowchart_completion: {
                  icon: BLOCK_TYPE_INFO.flowchart_completion.icon,
                  label: BLOCK_TYPE_INFO.flowchart_completion.label,
                  color: "text-green-500",
                },
                diagram_labelling: {
                  icon: BLOCK_TYPE_INFO.diagram_labelling.icon,
                  label: BLOCK_TYPE_INFO.diagram_labelling.label,
                  color: "text-green-400",
                },
                short_answer: {
                  icon: BLOCK_TYPE_INFO.short_answer.icon,
                  label: BLOCK_TYPE_INFO.short_answer.label,
                  color: "text-green-800",
                },
                essay_writing: {
                  icon: BLOCK_TYPE_INFO.essay_writing.icon,
                  label: BLOCK_TYPE_INFO.essay_writing.label,
                  color: "text-green-600",
                },
                letter_email_writing: {
                  icon: BLOCK_TYPE_INFO.letter_email_writing.icon,
                  label: BLOCK_TYPE_INFO.letter_email_writing.label,
                  color: "text-green-700",
                },
                paragraph_reordering: {
                  icon: BLOCK_TYPE_INFO.paragraph_reordering.icon,
                  label: BLOCK_TYPE_INFO.paragraph_reordering.label,
                  color: "text-green-500",
                },
                sentence_reordering: {
                  icon: BLOCK_TYPE_INFO.sentence_reordering.icon,
                  label: BLOCK_TYPE_INFO.sentence_reordering.label,
                  color: "text-green-400",
                },
                describe_picture: {
                  icon: BLOCK_TYPE_INFO.describe_picture.icon,
                  label: BLOCK_TYPE_INFO.describe_picture.label,
                  color: "text-green-800",
                },
                write_sentence_given_words: {
                  icon: BLOCK_TYPE_INFO.write_sentence_given_words.icon,
                  label: BLOCK_TYPE_INFO.write_sentence_given_words.label,
                  color: "text-green-600",
                },

                // Speaking
                cue_card_speaking: {
                  icon: BLOCK_TYPE_INFO.cue_card_speaking.icon,
                  label: BLOCK_TYPE_INFO.cue_card_speaking.label,
                  color: "text-red-600",
                },
                read_aloud: {
                  icon: BLOCK_TYPE_INFO.read_aloud.icon,
                  label: BLOCK_TYPE_INFO.read_aloud.label,
                  color: "text-red-700",
                },
                listen_and_speak: {
                  icon: BLOCK_TYPE_INFO.listen_and_speak.icon,
                  label: BLOCK_TYPE_INFO.listen_and_speak.label,
                  color: "text-red-500",
                },
                read_and_speak: {
                  icon: BLOCK_TYPE_INFO.read_and_speak.icon,
                  label: BLOCK_TYPE_INFO.read_and_speak.label,
                  color: "text-red-400",
                },
                speak_about_topic: {
                  icon: BLOCK_TYPE_INFO.speak_about_topic.icon,
                  label: BLOCK_TYPE_INFO.speak_about_topic.label,
                  color: "text-red-800",
                },
                audio_recording_response: {
                  icon: BLOCK_TYPE_INFO.audio_recording_response.icon,
                  label: BLOCK_TYPE_INFO.audio_recording_response.label,
                  color: "text-red-600",
                },
                impromptu_speech: {
                  icon: BLOCK_TYPE_INFO.impromptu_speech.icon,
                  label: BLOCK_TYPE_INFO.impromptu_speech.label,
                  color: "text-red-700",
                },
                pronunciation_correction: {
                  icon: BLOCK_TYPE_INFO.pronunciation_correction.icon,
                  label: BLOCK_TYPE_INFO.pronunciation_correction.label,
                  color: "text-red-500",
                },
                shadowing_practice: {
                  icon: BLOCK_TYPE_INFO.shadowing_practice.icon,
                  label: BLOCK_TYPE_INFO.shadowing_practice.label,
                  color: "text-red-400",
                },

                // Listening
                listen_and_type: {
                  icon: BLOCK_TYPE_INFO.listen_and_type.icon,
                  label: BLOCK_TYPE_INFO.listen_and_type.label,
                  color: "text-purple-600",
                },
                listen_and_select: {
                  icon: BLOCK_TYPE_INFO.listen_and_select.icon,
                  label: BLOCK_TYPE_INFO.listen_and_select.label,
                  color: "text-purple-700",
                },
                read_and_select: {
                  icon: BLOCK_TYPE_INFO.read_and_select.icon,
                  label: BLOCK_TYPE_INFO.read_and_select.label,
                  color: "text-purple-500",
                },
                read_and_complete: {
                  icon: BLOCK_TYPE_INFO.read_and_complete.icon,
                  label: BLOCK_TYPE_INFO.read_and_complete.label,
                  color: "text-purple-400",
                },

                // Grammar & Vocabulary
                identify_grammar_error: {
                  icon: BLOCK_TYPE_INFO.identify_grammar_error.icon,
                  label: BLOCK_TYPE_INFO.identify_grammar_error.label,
                  color: "text-orange-600",
                },
                choose_correct_tense: {
                  icon: BLOCK_TYPE_INFO.choose_correct_tense.icon,
                  label: BLOCK_TYPE_INFO.choose_correct_tense.label,
                  color: "text-orange-700",
                },
                find_synonym: {
                  icon: BLOCK_TYPE_INFO.find_synonym.icon,
                  label: BLOCK_TYPE_INFO.find_synonym.label,
                  color: "text-orange-500",
                },
                choose_best_title: {
                  icon: BLOCK_TYPE_INFO.choose_best_title.icon,
                  label: BLOCK_TYPE_INFO.choose_best_title.label,
                  color: "text-orange-400",
                },
                categorization_drag_drop: {
                  icon: BLOCK_TYPE_INFO.categorization_drag_drop.icon,
                  label: BLOCK_TYPE_INFO.categorization_drag_drop.label,
                  color: "text-orange-800",
                },
                insert_sentence_paragraph: {
                  icon: BLOCK_TYPE_INFO.insert_sentence_paragraph.icon,
                  label: BLOCK_TYPE_INFO.insert_sentence_paragraph.label,
                  color: "text-orange-600",
                },
                word_family_practice: {
                  icon: BLOCK_TYPE_INFO.word_family_practice.icon,
                  label: BLOCK_TYPE_INFO.word_family_practice.label,
                  color: "text-orange-700",
                },

                // Advanced & AI
                ai_feedback_writing: {
                  icon: BLOCK_TYPE_INFO.ai_feedback_writing.icon,
                  label: BLOCK_TYPE_INFO.ai_feedback_writing.label,
                  color: "text-pink-600",
                },
                image_based_prompt: {
                  icon: BLOCK_TYPE_INFO.image_based_prompt.icon,
                  label: BLOCK_TYPE_INFO.image_based_prompt.label,
                  color: "text-pink-700",
                },
                timed_task_exam: {
                  icon: BLOCK_TYPE_INFO.timed_task_exam.icon,
                  label: BLOCK_TYPE_INFO.timed_task_exam.label,
                  color: "text-pink-500",
                },
              };

              const blockInfo =
                blockTypeInfo[block.type as keyof typeof blockTypeInfo];
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <h4 className="font-semibold truncate">
                            {block.content?.title ||
                              `${blockInfo?.label || block.type} Block`}
                          </h4>
                          <Badge
                            variant="outline"
                            className="text-xs bg-gradient-to-r from-primary/5 to-violet-600/5 border-primary/20"
                          >
  ...
  const calculateCourseStats = (
    structure: any[]
  ): { totalLessons: number; totalSubcategories: number; maxDepth: number } => {
    let totalLessons = 0;
  ...
  const renderStructureMode = () => {
    const stats = selectedCourse?.structure
      ? calculateCourseStats(selectedCourse.structure)
      : { totalLessons: 0, totalSubcategories: 0, maxDepth: 0 };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="border border-border rounded-xl bg-gradient-to-br from-card to-card/50 p-6">
          ...
          {selectedCourse?.structure && selectedCourse.structure.length > 0 ? (
            <div className="space-y-2">
              {selectedCourse.structure.map((item: any) =>
                renderStructureItem(item, selectedCourse.id, 0)
              )}
            </div>
          ) : (
            ...
            <div className="flex-1">
              ...
              <ul className="text-sm text-muted-foreground space-y-2">
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    <strong>Nested Categories:</strong> Create unlimited levels
                ...
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-0.5">•</span>
                  <span>
                    <strong>Navigate:</strong> Click subcategories to
                    expand/collapse, click lessons to edit
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-0.5">•</span>
                  <span>
                    <strong>Breadcrumbs:</strong> Use the breadcrumb trail to
                    see your location and navigate back
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-0.5">•</span>
                  <span>
                    <strong>Add Content:</strong> Click "Add to [name]" inside
                    categories or use the dropdown menu
                  </span>
                </li>
              </ul>
            </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full min-h-0 bg-background -m-4 lg:-m-6">
          ...
          <div className="p-4 lg:p-6">
            ...
            {!selectedCourse ? (
              <div className="max-w-4xl mx-auto">
                <div className="text-center mb-8">
                  ...
                  <p className="text-muted-foreground">
                    Choose an existing course or create a new one to start
                    building
                  </p>
                </div>
              ...
              <>
                ...
                <div className="max-w-5xl mx-auto">{renderLessonCanvas()}</div>
              </>
            )}
          </div>
        </div>
      ...
      <AddContentModal
        ...
        onClose={() =>
          setAddContentModal({ isOpen: false, courseId: "", parentId: null })
        }
